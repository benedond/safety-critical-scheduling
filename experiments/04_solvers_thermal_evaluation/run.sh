...
run () {
	...
	for benchmark in $INPATH/*; do
		...
		if [ -f $OUTPATH/$CURBENCH.csv ]; then
	        ...
	        cd $AUTOBENCH_HOME
			thermobench --verbose --time=1800 --fan-cmd=fan --fan-on=0 --wait=50 --wait-timeout=60 --sensors_file=/root/thermobench/src/sensors.$PLAT --cpu-usage --column=CPU0_work_done --column=CPU1_work_done --column=CPU2_work_done --column=CPU3_work_done --column=CPU4_work_done --column=CPU5_work_done --output=$OUTPATH/$CURBENCH.csv -- demos-sched -p per_process -c $INPATH/$CURBENCH
		fi
	done
