...
do
    echo $i    
    if [ ! -f $i-bap-support-d9.out ]; then
        ../../../tools/src/decomposed_solver/main.py --input $i --output $i-bap-support-d9.out --log --branching supports --timelimit 3600 --depth 9 2> $i-bap-support-d9.err 1> $i-bap-support-d9.log
        ../../../tools/bin/visualizer.exe --input $i-bap-support-d9.out --output $i-bap-support-d9.png
    fi 
done