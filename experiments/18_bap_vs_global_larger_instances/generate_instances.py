...
if __name__ == "__main__":
    ...
    if not os.path.exists(dest):
        ...
        with open(os.path.join(dest,"RUN.sh"), "w") as f:
            ...
            f.write("        ../../../tools/src/ilp_global_solver/main.py --input $i --output $i-global.out --timelimit 600 2> $i-global.err 1> $i-global.log\n")
            f.write("        ../../../tools/bin/visualizer.exe --input $i-global.out --output $i-global.png\n")
            f.write("    fi \n")
            f.write("    if [ ! -f $i-bap.out ]; then\n")
            f.write("        ../../../tools/src/decomposed_solver/main.py --input $i --output $i-bap.out --log --branching tasks --timelimit 600 2> $i-bap.err 1> $i-bap.log\n")
