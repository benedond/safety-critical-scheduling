...
solution solve(const arg_parser& args, const environment& e, const assignment_characteristic_list& assignment_characteristics)
{
            ...
            for (int i = 0; i < num_tasks; i++)
            {
                ...
                for (auto& assignment_characteristic : task.resource_assignments)
                {
                    if (a[i][j][k++].get(GRB_DoubleAttr_X) > 0.5)
                    {
                        ...
                        {
                            win.task_assignments.push_back({ .task = task.task,
...
int main(int argc, char** argv)
{
    ...
    try
    {
        s = solve(args, environment, assignment_characteristics);
